...
param.val$true.mu1 = param.val$true.mu0+param.val$true.beta*param.val$b^2    
param.val$true.alpha = 0

param.val$alpha.ini.lb = param.val$true.alpha-5
param.val$alpha.ini.ub = param.val$true.alpha+5
param.val$beta.ini.lb = param.val$true.beta-5
param.val$beta.ini.ub = param.val$true.beta+5
param.val$mu0.ini.lb = param.val$true.mu0-5
param.val$mu0.ini.ub = param.val$true.mu0+5
param.val$mu1.ini.lb = param.val$true.mu1-5
param.val$mu1.ini.ub = param.val$true.mu1+5
...
data.generation = function(seed,param.val){
  ...
  ### data for proband 
  z0 = rLaplace(n=n0,param.val$true.mu0,param.val$b)
  
  z = rLaplace(n=10000,param.val$true.mu0,param.val$b)
  ...
  x1[u1<cut1[1]] = rLaplace(n=n11,param.val$true.mu0,param.val$b)
  y1[u1<cut1[1]] = rLaplace(n=n11,param.val$true.mu0,param.val$b)
  
  x1[u1>cut1[1] & u1<=cut1[2]]= rLaplace(n=n12,param.val$true.mu0,param.val$b)
  z = rLaplace(n=10000,param.val$true.mu0,param.val$b)
  p = exp(param.val$true.beta*z)/sum(exp(param.val$true.beta*z))
  D = rmultinom(n12,1,p)
  id = apply(D,2,function(x) which(x==1))
  y1[u1>cut1[1] & u1<=cut1[2]] = z[id]
   
  z = rLaplace(n=10000,param.val$true.mu0,param.val$b)
  p = exp(param.val$true.beta*z)/sum(exp(param.val$true.beta*z))
  D = rmultinom(n13,1,p)
  id = apply(D,2,function(x) which(x==1))
  x1[u1>cut1[2] & u1<=cut1[3]] = z[id]
  y1[u1>cut1[2] & u1<=cut1[3]]=rLaplace(n=n13,param.val$true.mu0,param.val$b)
  ...
  x0[u0<cut0[1]] = rLaplace(n=n01,param.val$true.mu0,param.val$b)
  y0[u0<cut0[1]] = rLaplace(n=n01,param.val$true.mu0,param.val$b)
  
  x0[u0>cut0[1] & u0<=cut0[2]]= rLaplace(n=n02,param.val$true.mu0,param.val$b)
  z = rLaplace(n=10000,param.val$true.mu0,param.val$b)
  p = exp(param.val$true.beta*z)/sum(exp(param.val$true.beta*z))
  D = rmultinom(n02,1,p)
  id = apply(D,2,function(x) which(x==1))
  y0[u0>cut0[1] & u0<=cut0[2]] = z[id]
  
  z = rLaplace(n=10000,param.val$true.mu0,param.val$b)
  p = exp(param.val$true.beta*z)/sum(exp(param.val$true.beta*z))
  D = rmultinom(n03,1,p)
  id = apply(D,2,function(x) which(x==1))
  x0[u0>cut0[2] & u0<=cut0[3]] = z[id]
  y0[u0>cut0[2] & u0<=cut0[3]]=rLaplace(n=n03,param.val$true.mu0,param.val$b)
...
obj.parametric.trio = function(par,t.list,n){
  #t.list = t.values[[j]]; par = ini.val.ms
  t = t.list[[1]]
  x0 = t.list[[2]]
  y0 = t.list[[3]]
  x1 = t.list[[4]]
...
f.tryCatch.parametric.trio = function(t.par,init){
  #t.par = t.values[[j]]; init = ini.val.ms
  tryCatch(expr = {
...
B = 4 ## bound for beta
k = 50 ## Number of random initital values, 20+1
...
for (j in 1:rep0){
  ...
  while(count==0){
    ...
    count = ifelse(is.na(val.parametric.trio),0,1)*ifelse(is.na(val.parametric.pairs),0,1)*ifelse(is.na(val.parametric.just.moth),0,1)
  }
  for(jj in 1:k)
  {
    #jj = 1
    ...
    while(count==0){
      ...
      count = ifelse(is.na(val1.parametric.trio),0,1)*ifelse(is.na(val1.parametric.pairs),0,1)*ifelse(is.na(val1.parametric.just.moth),0,1)
    }
    if(val1 < val)
    {
      temp = temp1
...
write.csv(est,"aUnknown_est.semiparametric_proband_genotype_n1000_beta0.3_a0.1.csv")
write.csv(est.parametric.trio,"aUnknown_est.parametric.trio_proband_genotype_n1000_beta0.3_a0.1.csv")
write.csv(est.parametric.pairs,"aUnknown_est.parametric.pairs_proband_genotype_n1000_beta0.3_a0.1.csv")
write.csv(est.parametric.just.moth,"aUnknown_est.parametric.just.moth_proband_genotype_n1000_beta0.3_a0.1.csv")
