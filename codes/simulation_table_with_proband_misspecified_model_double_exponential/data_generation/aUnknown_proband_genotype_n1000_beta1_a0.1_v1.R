...
for (j in 1:rep0){
  ...
  while(count==0){
    ...
    count = ifelse(is.na(val),0,1)*ifelse(is.na(val.parametric.trio),0,1)*ifelse(is.na(val.parametric.pairs),0,1)*ifelse(is.na(val.parametric.just.moth),0,1)
  }
  for(jj in 1:k)
  {
    #jj = 1
    ...
    while(count==0){
      ...
      count = ifelse(is.na(val1),0,1)*ifelse(is.na(val1.parametric.trio),0,1)*ifelse(is.na(val1.parametric.pairs),0,1)*ifelse(is.na(val1.parametric.just.moth),0,1)
    }
    if(val1 < val)
    {
      temp = temp1
