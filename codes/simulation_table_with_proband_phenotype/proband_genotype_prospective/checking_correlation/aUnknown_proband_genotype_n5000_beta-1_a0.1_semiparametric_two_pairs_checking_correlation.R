...
#### semi-parametric approach
lagrange.parameter.est = function(par,t.list,n){
  #t.list = t.values[[1]]; par = ini.val.ab
  t = t.list[[1]]
  x0 = t.list[[2]]
  y0 = t.list[[3]]
...
f.tryCatch.semiparametric.pairs = function(t.par,init){
  ...
  tryCatch(expr = {
    ...
    te=-B+2*B/(1+tt[2]^2)
    ta = tt[3]^2/(1+tt[3]^2)
    tau = lagrange.parameter.est(init,t.par,param.val)
    c(tt[1],te,ta,test$counts,test$convergence,test$value,tau)
...
rep0 = 500 ## number of repetitions
B = 4 ## bound for beta
...
est.semiparametric.pairs = matrix(0,rep0,11)
for (j in 1:rep0){
  #j=1
  print(j)
  count = 0
...
#output <- matrix(unlist(est.semiparametric.pairs), nrow = 200, byrow = F)
#est.semiparametric.pairs = output
#est.semiparametric.pairs = est.semiparametric.pairs[,-12]

est.semiparametric.pairs = as.data.frame(est.semiparametric.pairs)
colnames(est.semiparametric.pairs) = c("alpha.ini","beta.ini","a.ini","alpha.est","beta.est","a.est","iteration","nothing","convergence.code","min.value","tau.est")
...
a.est = est.semiparametric.pairs$a.est

lambda1_1m.par = (-a.est^3+a.est^2+a.est)/(a.est*(2-a.est))
lambda1_0m.par = 1-lambda1_1m.par

#lambda for Gp=0
lambda0_1m.par = a.est
lambda0_0m.par = 1-lambda0_1m.par

#### sister part
#lambda for Gp=1
lambda1_1s.par = (a.est^4-6*a.est^3+5*a.est^2+4*a.est)/(4*a.est*(2-a.est))
lambda1_0s.par = 1-lambda1_1s.par

#lambda for Gp=0
lambda0_1s.par = (-a.est^4+6*a.est^3-9*a.est^2+4*a.est)/(4*(1-a.est)^2)
lambda0_0s.par = 1-lambda0_1s.par

n1.n0 = lapply(t.values, "[[",6)
n0 = unlist(lapply(n1.n0,"[[",1))
n1 = unlist(lapply(n1.n0,"[[",2))

est.semiparametric.pairs$tau.reparam = est.semiparametric.pairs$tau.est-
  (n1+(1-lambda0_0m.par)*n0+(1-lambda1_0m.par)*n1+(1-lambda0_0s.par)*n0+(1-lambda1_0s.par)*n1)/(3*param.val$n)

cor.matrix = round(cor(est.semiparametric.pairs[,c(6,4,5,11)]),3)
#cor.matrix
cor.matrix_tau.reparam = round(cor(est.semiparametric.pairs[,c(6,4,5,12)]),3)


write.csv(cor.matrix,"cor.matrix_n5000.csv")
write.csv(cor.matrix_tau.reparam,"cor.matrix_tau.reparam_n5000.csv")
write.csv(est.semiparametric.pairs,"aUnknown_est.semiparametric.pairs_proband_genotype_n5000_beta-1_a0.1.csv")
