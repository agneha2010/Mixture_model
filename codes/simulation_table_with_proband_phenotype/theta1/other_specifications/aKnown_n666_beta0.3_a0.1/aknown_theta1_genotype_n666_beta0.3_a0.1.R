...
param.val = list(n = 666, ### proband size
                 a = 0.1,
                 true.mu0 = 1.7,
                 true.mu1 = 2,
                 true.sigma = 1,
...
data.generation = function(seed,param.val){
  ...
  lambda_11 = (-a^3-4*a^2+7*a+2)/(4*(2-a))
  lambda_10 = (2+a)*(1-a)^2/(4*(2-a))
  lambda_01 = (1+a)*(1-a)^2/(2*(2-a))
  lambda_00 = (1-a)^3/(2*(2-a))
...
obj = function(par,t,n){
  ...
  lambda_11.par = (-a.par^3-4*a.par^2+7*a.par+2)/(4*(2-a.par))
  lambda_10.par = (2+a.par)*(1-a.par)^2/(4*(2-a.par))
  lambda_01.par = (1+a.par)*(1-a.par)^2/(2*(2-a.par))
  lambda_00.par = (1-a.par)^3/(2*(2-a.par))
  ...
  value=sum(log(tm))+sum(tp)-sum(log(1+tt*te))
  return(-value)
}
...
obj.parametric.trio = function(par,t,n){
  ...
  value = sum(log(cut0.par[4]*dnorm(x,mu1.par,sigma.par)*dnorm(y,mu1.par,sigma.par)+
          ...
          sum(dnorm(z,mu1.par,sigma.par,log=T))
  return(-value)
}
...
obj.parametric.pairs = function(par,t,n){
  ...
  value = tm1+ts1+sum(dnorm(z,mu1.par,sigma.par,log=T))
...
obj.parametric.just.moth = function(par,t,n){
  ...
  value = ts1+sum(dnorm(z,mu1.par,sigma.par,log=T))
  return(-ts1)
}
...
B = 4 ## bound for beta
k = 20 ## Number of random initital values
...
write.csv(est,"aknown_est.semiparametric_theta1_genotype_n666_beta0.3_a0.1.csv")
write.csv(est.parametric.trio,"aknown_est.parametric.trio_theta1_genotype_n666_beta0.3_a0.1.csv")
write.csv(est.parametric.pairs,"aknown_est.parametric.pairs_theta1_genotype_n666_beta0.3_a0.1.csv")
write.csv(est.parametric.just.moth,"aknown_est.parametric.just.moth_theta1_genotype_n666_beta0.3_a0.1.csv")
