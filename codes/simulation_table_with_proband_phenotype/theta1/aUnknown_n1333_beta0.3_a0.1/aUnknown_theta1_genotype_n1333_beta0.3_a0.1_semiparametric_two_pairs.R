...
obj.semiparametric.pairs = function(par,t,n){
  ...
  t1=(cut1m.par[1]+cut1m.par[2]*exp(alp+x*bet))*(cut1s.par[1]+cut1s.par[2]*exp(alp+y*bet))
  tp=alp+z*bet
  
  value= sum(log(t1))+sum(tp)-sum(log(1+tt*te))
  return(-value)
}
...
write.csv(est.semiparametric.pairs,"aUnknown_est.semiparametric.pairs_theta1_genotype_n1333_beta0.3_a0.1.csv")
